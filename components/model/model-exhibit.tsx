...
const severityConfig: Record<string, { label: string; color: string; bg: string }> = {
  ...
  scandal: { label: "SCANDAL", color: "text-orange-400", bg: "bg-orange-500/10 border-orange-500/20" },
  ...
  political: { label: "POLITICAL", color: "text-rose-400", bg: "bg-rose-500/10 border-rose-500/20" },
  ...
  philosophical: { label: "PHILOSOPHICAL", color: "text-primary", bg: "bg-primary/10 border-primary/20" },
  overload: { label: "OVERLOAD", color: "text-orange-400", bg: "bg-orange-500/10 border-orange-500/20" },
  ironic: { label: "IRONIC", color: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20" },
}
    ...
    <div className="min-h-screen pt-16">
      <div className="mx-auto max-w-4xl px-4 pb-24 pt-10">
        ...
        <div className={cn(
          ...
          {/* Top accent bar */}
          <div className="h-[3px]" style={{ background: `linear-gradient(90deg, ${model.color}, transparent)` }} />
          ...
          <div className="p-6">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex flex-wrap gap-1.5">
                <span className="border border-dashed px-2 py-0.5 font-mono text-[10px]" style={{ color: category.color, borderColor: `${category.color}40` }}>
                  {category.icon} {category.label}
                </span>
                <span className={cn(
                  "border border-dashed px-2 py-0.5 font-mono text-[10px]",
                  model.status === "active" ? "border-primary/30 text-primary" : model.status === "declining" ? "border-orange-500/30 text-orange-400" : "border-border text-muted-foreground"
                )}>
                  {model.status === "active" ? "[ACTIVE]" : model.status === "declining" ? "[DECLINING]" : "[HISTORIC]"}
                </span>
                <span className={cn(
                  "border border-dashed px-2 py-0.5 font-mono text-[10px]",
                  model.open ? "border-emerald-500/30 text-emerald-400" : "border-border text-muted-foreground"
                )}>
            ...
            {/* Model name */}
            <h1 className="mt-4 text-3xl font-light tracking-tight text-foreground md:text-4xl">{model.name}</h1>
            ...
            <div className="mt-4 grid grid-cols-2 gap-px border border-dashed border-border sm:grid-cols-4">
              {[
                ...
                <div key={cell.label} className="border border-dashed border-border bg-background/50 px-3 py-2.5">
                  <p className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">{cell.label}</p>
                  <p className="mt-0.5 font-mono text-xs tabular-nums text-foreground">{cell.value}</p>
                </div>
              ))}
            </div>
            ...
            <p className="mt-3 font-mono text-[10px] text-muted-foreground/50">
              ID: {model.id} | ERA: {model.era} | IDX: {String(modelIndex + 1).padStart(2, "0")}/{String(models.length).padStart(2, "0")}
            </p>
          </div>
        </div>
        ...
        {/* ===== TAB CONTENT ===== */}
        <div ref={contentRef} className="mt-4 animate-[terminalFadeIn_0.3s_ease-out]" key={activeTab}>
          ...
          {activeTab === "overview" && (
            <div className="space-y-4">
              ...
              <div className="terminal-card-solid p-5">
                ...
                <p className="text-sm leading-[1.7] text-foreground/85 whitespace-pre-line">{model.description}</p>
              </div>

              {/* Example output as styled quote */}
              {model.example && (
                <div className="relative terminal-card-solid overflow-hidden p-5">
                  <div className="absolute left-0 top-0 bottom-0 w-[2px]" style={{ backgroundColor: model.color }} />
                  <p className="data-label mb-3">[Example Output]</p>
                  <div className="pl-3">
                    <p className="font-mono text-[13px] leading-[1.8] text-foreground/80 whitespace-pre-line italic">{model.example}</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  <div key={bar.label} className="terminal-card-solid p-4" style={{ transitionDelay: `${200 + i * 100}ms` }}>
          ...
          {activeTab === "stats" && (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {[
                ...
                { label: "License", value: model.open ? "Open Source" : "Proprietary", mono: false },
                ...
                >
                  ...
                  <p className={cn("mt-1.5 text-lg tabular-nums text-foreground", stat.mono && "font-mono")}>{stat.value}</p>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="space-y-2">
              {model.opinions.length === 0 ? (
                <div className="terminal-card-solid p-10 text-center">
                  <p className="font-mono text-sm text-muted-foreground">No opinions logged for this model.</p>
                </div>
              ) : (
                model.opinions.map((op, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="terminal-card-solid relative overflow-hidden p-4"
                    >
                      {/* Accent side bar */}
                      <div className={cn("absolute left-0 top-0 bottom-0 w-[2px]", isPositive ? "bg-emerald-500" : "bg-red-500")} />
                      {/* Sentiment badge */}
                      <span className={cn(
                        "inline-block border border-dashed px-2 py-0.5 font-mono text-[10px]",
                        isPositive ? "border-emerald-500/30 text-emerald-400" : "border-red-500/30 text-red-400"
                      )}>
                      ...
                      <p className="mt-2 font-mono text-[11px] text-muted-foreground">
                        -- {op.source}
                        {op.url && (
                          <a href={op.url} target="_blank" rel="noopener noreferrer" className="ml-1.5 inline-flex items-center gap-0.5 text-primary hover:underline">
          ...
          {activeTab === "bugs" && (
            <div className="space-y-2">
              {model.bugs.length === 0 ? (
                <div className="terminal-card-solid p-10 text-center">
                  <p className="font-mono text-sm text-muted-foreground">Clean record. No incidents to report.</p>
                </div>
              ) : (
                model.bugs.map((bug, i) => {
                  ...
                  return (
                    <div key={i} className="terminal-card-solid p-4">
                      <span className={cn("inline-block border px-2 py-0.5 font-mono text-[10px]", sev.color, sev.bg)}>
                        [{sev.label}]
                      </span>
          ...
          {activeTab === "media" && (
            <div className="space-y-2">
              {model.media.length === 0 ? (
                <div className="terminal-card-solid p-10 text-center">
                  <p className="font-mono text-sm text-muted-foreground">No media artifacts available.</p>
                </div>
              ) : (
                model.media.map((m, i) => (
                  <a
          ...
          <div className="mt-4 flex items-center gap-0.5">
            ...
            {[1, 2, 3, 4, 5].map((s) => (
              <button
                key={s}
                onClick={() => setRating(s)}
                className={cn("transition-all duration-150 hover:scale-110", s <= rating ? "text-yellow-400" : "text-muted-foreground/20")}
              >
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      count > 0 ? "border-primary/20 text-foreground" : "border-border text-muted-foreground"
                    )}
                  >
        ...
        <div className="mt-6 grid grid-cols-3 gap-2">
          {prevModel ? (
            ...
            >
              <ChevronLeft size={14} className="shrink-0 text-muted-foreground transition-transform group-hover:-translate-x-0.5" />
