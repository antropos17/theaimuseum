...
const ERA_THEMES: Record<
  string,
  { text: string; prompt: string; accent: string; bg: string; label: string }
> = {
  '1966': {
    text: 'text-green-400',
    ...
    bg: 'bg-green-500',
    label: 'ELIZA // Weizenbaum Labs // 1966',
  },
  '2020': {
    text: 'text-amber-300',
  ...
  '2024': {
    ...
    bg: 'bg-purple-500',
    label: 'Claude 3 / GPT-4 // 2024',
  },
  '2025': {
    text: 'text-rose-300',
    prompt: 'text-rose-200',
    accent: 'text-rose-500',
    bg: 'bg-rose-500',
    label: 'o3 / DeepSeek R1 // 2025',
  },
}
...
export function SimulatorView() {
  ...
  function selectPrompt(pIdx: number) {
    ...
    typeText(prompt.prompt, 25, setTypedText, () => {
      // Commit user line
      setLines((prev) => [...prev, { type: 'user', text: prompt.prompt }])
      setTypedText('')
      ...
      <div className="mx-auto max-w-3xl px-4 pb-24 pt-10">
        ...
        <div className="mt-5 overflow-hidden border border-border bg-[#0a0a0f]">
          ...
          <div className="flex items-center justify-between border-b border-border/50 bg-[#111118] px-4 py-2">
            <div className="flex items-center gap-2">
              ...
              <span className={cn('font-mono text-[10px]', theme.accent)}>{theme.label}</span>
            </div>
          ...
          <div
            ...
            style={{
              ...
              textShadow: era.era === '1966' ? '0 0 6px rgba(74, 222, 128, 0.25)' : 'none',
            }}
          >
            {/* Rendered lines */}
            {lines.map((line, i) => {
              if (line.type === 'divider') {
                return <div key={i} className="my-2 border-t border-dashed border-border/30" />
              }
              if (line.type === 'system') {
                return (
              ...
              if (line.type === 'ai') {
                return (
                  <div
                    key={i}
                    className={cn(
                      'mb-1 whitespace-pre-wrap pl-2 border-l border-dashed',
                      theme.text,
                      `border-${theme.accent.replace('text-', '')}`,
                    )}
                  >
                    {line.text}
                  </div>
            ...
            {showPromptPicker && phase === 'idle' && remainingPrompts.length > 0 && (
              <div className="mt-3">
                <div className="mb-2 text-[10px] text-muted-foreground">[SELECT PROMPT]</div>
        ...
        <div className="mt-4 border border-dashed border-border p-4">
          ...
          <p className="mt-2 text-[13px] leading-relaxed text-muted-foreground">
            {era.era === '1966' &&
              'ELIZA was one of the first chatbots, created at MIT. It used simple pattern matching to simulate a Rogerian psychotherapist. Users were often convinced they were talking to a real person.'}
            {era.era === '2020' &&
              'GPT-3 stunned the world with 175B parameters. It could write essays, code, and poetry -- but often lost coherence in longer outputs and confidently stated falsehoods.'}
            {era.era === '2022' &&
              'ChatGPT brought AI to the mainstream. 100M users in 2 months. It could hold conversations, admit mistakes, and refuse harmful requests -- a massive leap in usability.'}
            {era.era === '2024' &&
              'Claude 3 and GPT-4 represent frontier models with near-human reasoning. They can analyze documents, debug code, and provide nuanced analysis with far fewer hallucinations.'}
            {era.era === '2025' &&
              'o3 and DeepSeek R1 push reasoning further with chain-of-thought approaches. DeepSeek trained for $5.6M vs billions for competitors, proving efficiency can rival brute force.'}
          </p>
        </div>
